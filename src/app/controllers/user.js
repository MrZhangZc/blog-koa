...
export const userList = async ctx => {
    ...
    for(let i of users){
      Object.assign(i, {times: await redisClient.bitcount(`${KEY.User_Login}${new Date().getFullYear()}_${new Date().getMonth() + 1}_${i.id}`)})
    }
...
export const loginPost = async ctx => {
	try {
      ...
      await redisClient.setbit(`${KEY.User_Login}${new Date().getFullYear()}_${new Date().getMonth() + 1}_${_user._id}`, new Date().getDate(), 1)
